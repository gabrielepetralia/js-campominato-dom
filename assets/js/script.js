...
btnFlag.classList.add("iconFlag")
btnFlag.addEventListener("click",function(){
  placeFlag = !placeFlag;
  if(placeFlag){
    this.classList.remove("iconFlag")
    this.classList.add("iconNumber")
    toggleCursorNotAllowed()
  } else {
    this.classList.remove("iconNumber")
    this.classList.add("iconFlag")
    toggleCursorNotAllowed()
  }
})
...
function handleClickCell(){
  ...
  if(placeFlag && !this.classList.contains("clicked")){
    this.classList.toggle("flag");
  } else {
    if(bombs.includes(this.cellId) && !this.classList.contains("flag")){
      this.classList.add("bomba");
      this.classList.remove("flag");
      endGame(false,cells.length);
    } else if(!this.classList.contains("clicked") && !this.classList.contains("flag")){
...
function toggleCursorNotAllowed(){
  const cells = document.getElementsByClassName("cella")
  for(let i = 0 ; i < cells.length ; i++){
    if(cells[i].classList.contains("flag")){
      if(placeFlag){
        cells[i].classList.remove("cursor-not-allowed")
      } else{
        cells[i].classList.add("cursor-not-allowed")
      }
    }
  }
}